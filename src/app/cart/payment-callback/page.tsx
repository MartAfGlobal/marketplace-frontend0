...
function PaymentCallbackContent() {
  ...
  useEffect(() => {
    ...
    if (!reference) {
      router.replace("/cart/payment-callback/failed");
      return;
    }
    ...
    getOrderRequest({
      ...
      successRes: (res: any) => {
        ...
        if (isSuccess) {
          ...
          router.replace(
            `/cart/payment-callback/success?ref=${encodeURIComponent(reference)}`
          );
        } else {
          router.replace(
            `/cart/payment-callback/failed?ref=${encodeURIComponent(reference)}`
          );
        }
      },
      errorRes: () => {
