...
export interface ConfirmRejectDisputeModalProps {
  isOpen: boolean;
  onClose: () => void;
  DisputeId?: string;
  onConfirm?: () => void;
...
export default function ConfirmRejectDisputeModal({
  isOpen,
  onClose,
  DisputeId,
  onConfirm,
  loading = false,
}: ConfirmRejectDisputeModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!entered) {
      setError("Please type the dispute ID to confirm rejection.");
      return;
    }
          ...
          <div className="space-y-8">
            <div className="text-center space-y-1">
              ...
              <p className="text-xs text-000000/68 font-MontserratNormal leading-[16px]">
                This will terminate the return/refund process for
              </p>
              {DisputeId && (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex gap-4 pt-4">
                ...
                <Button
                  ...
                  className="flex-1"
                >
                  {loading ? <LoadingSpinner color="border-white" /> : "Confirm rejection"}
                </Button>
              </div>
            </form>
          </div>
