...
const lightGray = "#f3f4f6"; // Tailwind gray-100 (very light)

const fullGraySlice = [{ renderValue: 100 }];
  const segments = [
  ...
  <>
  {isIncomplete? (
      <div className="w-full max-w-134.75 h-119 py-6 px-8 bg-ffffff circle-shadow rounded-c16">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="relative">
          <button
            onClick={() => setIsOpen(!isOpen)} disabled={isIncomplete}
      ...
      <PieChart>
        <Pie
          ...
          startAngle={90}
          endAngle={-270}
          cornerRadius={0}  // no rounding for uniform circle
          paddingAngle={0}  // no padding needed
        >
          <Cell fill={lightGray} />
        </Pie>
      </PieChart>
        ...
        <div className="mt-4" >
          {segments.map((seg, i) => (
            <div
              key={seg.label}
              className="w-full max-w-35 flex gap-4 text-000000/10 "
            >
              <div className="flex items-center gap-0.5 w-full    pb-3 max-w-23.25 text-nowrap">
                <div
                  className="h-1.5 w-1.5 rounded-full flex-shrink-0 bg-gray-200"
                  
                />
                <p className="text-c10 font-MontserratMedium text-000000/10 w-full">
                  No data
                </p>
              </div>
              <div className="w-full max-w-7.75 ">
                <p className="font-MontserratSemiBold text-c12">0</p>
              </div>
            </div>
          ))}
        </div>
