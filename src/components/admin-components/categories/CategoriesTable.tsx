      ...
      <table className="w-full text-left">
        <thead>
          <tr className="h-10.5 bg-947fff text-ffffff text-nowrap">
            <th className="font-MontserratNormal text-sm text-center w-10 p-3">
              <button
                type="button"
                aria-label={rows.length > 0 ? "Select all visible categories" : "No categories to select"}
                onClick={(e) => {
                  e.stopPropagation();
                  onSelectAll();
                }}
            ...
            <th className="p-3 font-MontserratNormal text-sm">Category Name</th>
            <th className="p-3 font-MontserratNormal text-sm">Subcategory</th>
        ...
        <tbody className="divide-y divide-gray-50 text-[11px] text-gray-700 font-MontserratMedium">
          {loading ? (
              ...
              >
                ...
                <td className="py-3 px-4 text-[#161616] font-MontserratSemiBold">
                  <div className="flex items-center gap-3">
                    <Image src={ProductImage} alt={row.name} width={28} height={28} className="rounded object-cover" />
                    <span className="block truncate" title={row.name}>{row.name}</span>
                  </div>
                </td>
                <td className="py-3 px-4 text-gray-700">
                  <span className="block max-w-[12rem] truncate" title={row.subcategories}>{row.subcategories}</span>
                </td>
                <td className="py-3 px-4 text-gray-700">
                  <span className="block max-w-[10rem] truncate" title={row.attributes}>{row.attributes}</span>
                </td>
                    ...
                    {activeRowId === row.id && (
                      ...
                      >
                        ...
                        <button
                          onClick={() => {
                            onSetActiveRowId(null);
                            toast.success(`Category ${row.name} status toggled.`);
                          }}
                          className="w-full text-left px-4 py-2 hover:bg-gray-50 text-gray-700 transition-colors cursor-pointer"
                        >
            ...
            <tr>
              <td colSpan={8} className="py-8 text-center text-gray-400 font-MontserratMedium text-xs">No records found matching your search.</td>
            </tr>
          )}
        </tbody>
      </table>
